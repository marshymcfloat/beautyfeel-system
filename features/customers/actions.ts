...
export async function redeemStoreCredit(input:unknown){return runAction(async()=>{const actor=await requireActor(["OWNER"]);const data=z.object({creditId:z.string().uuid(),bookingId:z.string().uuid(),amountCentavos:z.number().int().positive()}).parse(input);return prisma.$transaction(async tx=>{const[credit,booking]=await Promise.all([tx.storeCredit.findUnique({where:{id:data.creditId}}),tx.booking.findUnique({where:{id:data.bookingId}})]);if(!credit||!booking||credit.customerPhoneE164!==booking.customerPhoneE164||credit.status!=="ACTIVE"||credit.expiresAt<=new Date())throw new DomainError("VALIDATION_ERROR","Store credit cannot be applied.");const remaining=redeemCreditBalance(credit.remainingCentavos,data.amountCentavos);await tx.storeCredit.update({where:{id:credit.id},data:{remainingCentavos:remaining,status:remaining===0?"USED":"ACTIVE",usedAt:remaining===0?new Date():null}});await tx.auditLog.create({data:{actorId:actor.id,action:"STORE_CREDIT_REDEEMED",entityType:"StoreCredit",entityId:credit.id,metadata:{bookingId:booking.id,amountCentavos:data.amountCentavos}}});return{creditId:credit.id,remainingCentavos:remaining}})})}
